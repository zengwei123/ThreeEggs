...
public class AddressItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public AddressItemDecoration(Context context, OnGroupListener listener) {
        this.context = context;
        this.listener = listener;
        groupDividerHeight = dp2Px(35);
        initPaint();
    }
    ...
    public void onDraw(Canvas canvas, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (isGroupFirst(childAdapterPosition)) {   // 是分组第一个，则绘制分组分割线
                ...
                canvas.drawText(getGroupName(childAdapterPosition), left + dp2Px(10), baseLine, textPaint);
            }
        }
    }
