...
public class CircleRecyclerListPresenter extends BasePresenter<CircleRecyclerListView> {
    ...
    public void setView() {
        ...
        circleLists.add(new CircleList(R.mipmap.a6,"","他也真怪，即使在最睛朗的日子，也穿上雨鞋，带上雨伞，而且一定穿着暖和的棉大衣。他总是把雨伞装在套子里，把表放在一个灰色的鹿皮套子里；就连削铅笔的小刀也是装在一个小套子里的。他的脸也好像蒙着套子，因为他老是把它藏在竖起的衣领里。","爸爸爸爸",""));

        SimpleRecyclerViewAdapter simpleRecyclerViewAdapter=new SimpleRecyclerViewAdapter(
                R.layout.circlerecyclerlist_fragment_item, circleLists, CircleList.class, (helper, item) -> {
        ...
        mvpView.getCircleRecyclerList_Recycler().setLayoutManager(SimpleUtils.getRecyclerLayoutManager(mvpView.getActivityContext(),2));
    }
}
